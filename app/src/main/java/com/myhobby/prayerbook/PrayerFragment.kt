...
import com.myhobby.prayerbook.database.Prayer
...
class PrayerFragment : Fragment(), UserDB.Listener, MenuProvider {
        ...
        tts = TextToSpeech(requireContext()) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                    ...
                    // Set the voice if one was previously selected for this language
                    val voiceName = Prefs.get().getSpeechVoice(languageCode)
                    if (voiceName != null) {
    ...
    fun startSpeakingPrayer() {
        ...
        if (isSpeaking && !isPaused) {
            ...
            val langCode = Prefs.get().getLanguage()
            val voiceName = Prefs.get().getSpeechVoice(langCode)
            if (voiceName != null) {
                val voice = tts?.voices?.find { it.name == voiceName }
                if (voice != null) {
                    tts?.setVoice(voice)
